    ...
    public function actionIndex()
    {
        ...
        if ( !is_null(\Yii::$app->request->post('API_KEY',null))  && \Yii::$app->request->post('API_KEY') === \Yii::$app->params['API_KEY']) {
            ...
            if ($model->load(\Yii::$app->request->post())) {
                ...
                if (isset($model->pb_agent)) {
                    if (UserAccount::find()->where(['username' => strtoupper( $model->pb_agent )   ])->exists()) {
                        $userAccount = UserAccount::find()->where(['username' => strtoupper( $model->pb_agent ) ])->one();                    
                    }
                }
